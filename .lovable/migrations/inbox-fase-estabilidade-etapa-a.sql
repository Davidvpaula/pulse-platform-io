-- ============================================================
-- MIGRATION: Inbox Fase Estabilidade — Etapa A (schema)
-- Apenas estrutura. NÃO mexe em RLS de tabelas existentes.
-- NÃO altera dados. NÃO toca em edge functions.
-- ============================================================

-- ------------------------------------------------------------
-- A.2 Colunas aditivas (todas nullable, defaults seguros)
-- ------------------------------------------------------------
...
ALTER TABLE public.whatsapp_instances
  ...
  ADD COLUMN IF NOT EXISTS tenant_id uuid;  -- preparação multi-tenant (não usado ainda)

-- ------------------------------------------------------------
-- A.1 Tabela: conversation_pacientes (vínculo N:N auditável)
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.conversation_pacientes (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  conversation_id uuid NOT NULL REFERENCES public.conversations(id) ON DELETE CASCADE,
  paciente_id     uuid NOT NULL REFERENCES public.pacientes(id) ON DELETE CASCADE,
  parentesco      text,                       -- 'titular' | 'filho' | 'conjuge' | 'mae' | 'pai' | 'outro'
  origem          text NOT NULL DEFAULT 'manual', -- 'manual' | 'sugestao_aceita' | 'telefone_match'
  vinculado_por   uuid,                       -- auth.uid() do colaborador
  vinculado_em    timestamptz NOT NULL DEFAULT now(),
  confirmado_por  uuid,                       -- precisa confirmação para liberar prontuário
...
-- ------------------------------------------------------------
-- A.1 Tabela: conversation_meta_window (cache 24h)
-- ------------------------------------------------------------
...
-- INSERT/UPDATE só via trigger SECURITY DEFINER. Sem policy para usuário.

-- ------------------------------------------------------------
-- A.1 Tabela: conversation_audit_log (LGPD, append-only)
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.conversation_audit_log (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  conversation_id uuid NOT NULL REFERENCES public.conversations(id) ON DELETE CASCADE,
  actor_user_id   uuid,
  action          text NOT NULL,    -- 'assumir' | 'liberar' | 'transferir' | 'vincular_paciente' | 'confirmar_vinculo' | 'trocar_paciente_ativo' | 'leitura_prontuario'
...
-- ------------------------------------------------------------
-- A.4 Trigger: atualizar janela 24h em mensagens inbound
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- A.4 Trigger: auditoria de mudança de lock/assigned/paciente_ativo
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- A.4 Trigger: updated_at em conversation_pacientes
-- ------------------------------------------------------------
...
BEGIN
  ...
  IF NOT has_permission(v_uid, 'comunicacao.responder') AND NOT has_role(v_uid, 'admin') THEN
    RAISE EXCEPTION 'Sem permissão para assumir conversa' USING ERRCODE = '42501';
  END IF;
  ...
  IF v_conv.locked_by IS NOT NULL AND v_conv.locked_by <> v_uid AND NOT has_role(v_uid, 'admin') THEN
    RAISE EXCEPTION 'Conversa já está sendo atendida por outro colaborador' USING ERRCODE = '55006';
  END IF;
...
BEGIN
  ...
  IF v_conv.locked_by IS DISTINCT FROM v_uid AND NOT has_role(v_uid, 'admin') THEN
    RAISE EXCEPTION 'Apenas o atendente atual ou admin pode liberar' USING ERRCODE = '42501';
  END IF;
...
BEGIN
  ...
  IF p_to_user_id IS NULL AND p_to_sector IS NULL THEN
    RAISE EXCEPTION 'Informe destino (usuário ou setor)' USING ERRCODE = '22023';
  END IF;
  ...
  UPDATE public.conversations
     SET assigned_to     = COALESCE(p_to_user_id, assigned_to),
         ...
         locked_by       = NULL,
         locked_at       = NULL,
         status          = 'pendente'::conversation_status,
         updated_at      = now()
   WHERE id = p_conversation_id
   RETURNING * INTO v_conv;
  ...
  -- Audit log já registra automaticamente via trigger; aqui anexamos motivo
  INSERT INTO public.conversation_audit_log (conversation_id, actor_user_id, action, payload)
...
-- ------------------------------------------------------------
-- A.5 RPC: vincular_paciente_conversa (sem confirmar — sem prontuário ainda)
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.vincular_paciente_conversa(
  p_conversation_id uuid,
  p_paciente_id     uuid,
  p_parentesco      text DEFAULT 'titular',
  p_origem          text DEFAULT 'manual'
)
RETURNS public.conversation_pacientes
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_link public.conversation_pacientes;
  v_uid  uuid := auth.uid();
BEGIN
  IF v_uid IS NULL THEN
    RAISE EXCEPTION 'Não autenticado' USING ERRCODE = '28000';
  ...
  IF NOT has_permission(v_uid, 'comunicacao.responder') AND NOT has_role(v_uid, 'admin') THEN
    RAISE EXCEPTION 'Sem permissão para vincular paciente' USING ERRCODE = '42501';
  END IF;

  INSERT INTO public.conversation_pacientes
    (conversation_id, paciente_id, parentesco, origem, vinculado_por)
  VALUES
    (p_conversation_id, p_paciente_id, p_parentesco, p_origem, v_uid)
...
-- ------------------------------------------------------------
-- A.5 RPC: confirmar_vinculo_paciente (libera leitura clínica)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- A.5 RPC: definir_paciente_ativo_conversa
-- ------------------------------------------------------------
...
BEGIN
  ...
  IF p_paciente_id IS NOT NULL THEN
    SELECT EXISTS (
      SELECT 1 FROM public.conversation_pacientes
    ...
    IF NOT v_exists THEN
      RAISE EXCEPTION 'Paciente não está vinculado a esta conversa' USING ERRCODE = '23503';
    END IF;
  END IF;

  UPDATE public.conversations
     SET paciente_ativo_id = p_paciente_id,
         updated_at        = now()
   WHERE id = p_conversation_id
   RETURNING * INTO v_conv;
...
-- ------------------------------------------------------------
-- GRANTs (RPCs ficam acessíveis via PostgREST)
-- ------------------------------------------------------------
