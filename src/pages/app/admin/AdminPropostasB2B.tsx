    ...
    <div className="space-y-6">
      ...
      <div className="card-elevated overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {lista.length === 0 && (
              <tr><td colSpan={8} className="py-16 text-center text-muted-foreground">Nenhuma proposta encontrada.</td></tr>
            )}
            {lista.map(p => {
              ...
              return (
                <tr key={p.id} className="border-t border-border hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-3 py-3 text-muted-foreground">{p.especialidade_nome ?? "—"}</td>
            ...
            <div className="space-y-4 text-sm">
                ...
                <div className="space-y-3 border-t border-border pt-3">
                  ...
                  {taxaPct && (
                    <div className="rounded-lg bg-muted/40 p-3 text-xs space-y-1">
                      ...
                      <p className="font-semibold">Repasse médico: {brl(Math.round((valorAjustado ? parseFloat(valorAjustado.replace(",",".")) * 100 : selected.valor_mensal_centavos) * (1 - parseFloat(taxaPct) / 100)))}</p>
                    </div>
                  )}
              ...
              {selected.status !== "criada" && selected.status !== "em_analise" && (
                <div className="space-y-2 border-t border-border pt-3">
                  {selected.taxa_plataforma_pct != null && (
                    <p className="text-xs">Taxa aplicada: <span className="font-semibold">{selected.taxa_plataforma_pct}%</span></p>
                  )}
