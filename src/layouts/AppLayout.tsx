...
function SidebarBody({
  ...
  return (
    <>
      ...
      {/* Accordion: derive initially-open group from current pathname */}
      {(() => {
        const activeGroup = visibleNav.find(
          (item) => item.children?.some((c) => pathname.startsWith(c.to))
        )?.label ?? null;
        return <AccordionNav visibleNav={visibleNav} activeGroup={activeGroup} loading={loading} isDemoMode={isDemoMode} profileKey={profileKey} pathname={pathname} onNavigate={onNavigate} linkClass={linkClass} />;
      })()}
