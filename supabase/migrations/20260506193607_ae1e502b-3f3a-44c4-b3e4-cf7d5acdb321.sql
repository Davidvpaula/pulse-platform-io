...
-- Drop old function first (has p_medico parameter)
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'diario', COALESCE((
      SELECT json_agg(row_to_json(t) ORDER BY t.dia) FROM (
        SELECT cf.data_consulta::date as dia,
               SUM(cf.valor_bruto_centavos) as receita_bruta_centavos,
               SUM(cf.valor_medico_centavos) as receita_liquida_centavos
        FROM consultas_financeiro cf
        WHERE cf.status = 'valido'
          AND cf.data_consulta::date >= p_inicio
          AND cf.data_consulta::date <= p_fim
    ...
    'por_medico', COALESCE((
      SELECT json_agg(row_to_json(t)) FROM (
        SELECT cf.medico_id,
               ...
               SUM(cf.valor_plataforma_centavos) as comissao_centavos,
               SUM(cf.valor_medico_centavos) as repasse_centavos
        FROM consultas_financeiro cf
        LEFT JOIN medicos m ON m.id = cf.medico_id
        WHERE cf.status = 'valido'
          AND cf.data_consulta::date >= p_inicio
          AND cf.data_consulta::date <= p_fim
    ...
    'por_modalidade', COALESCE((
      SELECT json_agg(row_to_json(t)) FROM (
        SELECT COALESCE(c.modalidade::text, 'indefinida') as modalidade,
               COUNT(*) as consultas,
               SUM(cf.valor_bruto_centavos) as receita_centavos
        FROM consultas_financeiro cf
        LEFT JOIN consultas c ON c.id = cf.consulta_id
